...
async def resolve_token_id(session: aiohttp.ClientSession, token: str) -> str | None:
    """
    ...
    2. Cache z predošlých vyhľadávaní
    3. CoinGecko /search endpoint — nájde takmer akýkoľvek existujúci token

    Vráti None, ak sa token nenašiel vôbec.
    """
    ...
    token_id = await _search_token(session, token)
    _search_cache[token] = token_id
    return token_id


async def _search_token(session: aiohttp.ClientSession, query: str) -> str | None:
    """
    Zavolá CoinGecko /search a vyberie najpravdepodobnejšiu zhodu.
    Uprednostní token, ktorého symbol (ticker) presne sedí so zadaným textom
    (napr. zadanie 'pepe' by malo trafiť token so symbolom PEPE, nie niečo
    iné, čo má 'pepe' len v názve).
    """
    ...
    # 3. inak zober prvý výsledok — CoinGecko ich radí podľa relevancie/market cap
    return coins[0]["id"]
...
async def price(interaction: discord.Interaction, token: str):
    ...
    async with aiohttp.ClientSession() as session:
        token_id = await resolve_token_id(session, token)
        if token_id is None:
            await interaction.followup.send(
                f"❌ Nenašiel som token `{token}`. Skús presnejší názov alebo ticker."
            )
            return
    ...
    await interaction.followup.send(
        f"💰 **{token.upper()}**: ${current_price:,.4f}"
    )
...
async def alert(interaction: discord.Interaction, token: str, cena: float):
    ...
    await interaction.followup.send(
        f"✅ Alert nastavený! Upozorním ťa, keď **{token.upper()}** {smer} **${cena:,.2f}**\n"
        f"(aktuálna cena: ${current_price:,.4f})"
    )
...
async def myalerts(interaction: discord.Interaction):
    ...
    for a in user_alerts:
        ...
        lines.append(f"`#{a['id']}` {a['token_label']} {smer} ${a['target_price']:,.2f}")
        ...
        if hit:
            ...
            if channel:
                await channel.send(
                    ...
                    f"Aktuálna cena: **${current:,.4f}** (cieľ: ${a['target_price']:,.2f})"
                )
            a["triggered"] = True
            changed = True
